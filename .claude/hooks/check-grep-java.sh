...
if echo "$GLOB $PVAL $TVAL" | grep -qiE '(\.java|type.*java)'; then
  echo '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"BLOCKED: Use Serena MCP to search Java code: search_for_pattern(pattern) or find_symbol(name). See CLAUDE.md."}}'
fi
